...
async def on_startup():
    ...
    # Pre-warm detector (slow, so we start it early)
    threading.Thread(target=get_detector, daemon=True).start()
    ...
    # Ensure Tejas20 admin exists
    tejas = get_user_by_id("tejas20")
    if not tejas:
        print("Creating Tejas20 admin account...")
        create_user(
            first_name="Tejas",
            last_name="Admin",
            user_id="tejas20",
        ...
        # If user exists, ensure it has the admin role
        if tejas.get("role") != "admin":
            print("Promoting tejas20 to admin...")
            update_user_role("tejas20", "admin")
    ...
    try:
        ...
        while True:
            ...
            if frame_data:
                ...
                # Generate a "Please Wait" or "Stopped" placeholder frame
                if placeholder_frame is None:
                    img = np.zeros((480, 640, 3), dtype=np.uint8)
                    status_text = "Initializing Feed..." if system_running else "System Stopped"
                    cv2.putText(img, status_text, (150, 240), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                    cv2.putText(img, f"Lane {lane_id}", (10, 30), 
            ...
            await asyncio.sleep(0.5)  # Slower polling when no real frame
...
async def stop_system():
    global system_running, latest_frames
    system_running = False
    get_controller().stop()
    # Clear stale frames
    latest_frames = {1: None, 2: None, 3: None, 4: None}
...
if __name__ == "__main__":
    ...
    # In production (Render), we don't need reload=True
    # reload=True is mostly for local development
